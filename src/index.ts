...
function toText(data: unknown): TextResult {
  return {
    content: [{
      type: "text",
...
function assertGamePath(input: string, label: string): string {
...
function commandForScript(script: string): { command: string; args: string[] } {
  return {
    command: editorCmd,
...
function runProcess(command: string, args: string[], timeoutSeconds: number): Promise<RunResult> {
...
async function runUnrealScript(script: string, dryRun: boolean, timeoutSeconds: number) {
  if (!existsSync(uproject)) throw new Error(`Ascent uproject not found: ${uproject}`);
  if (!existsSync(editorCmd)) throw new Error(`UnrealEditor-Cmd not found: ${editorCmd}`);
  const { command, args } = commandForScript(script);
  if (dryRun) return { command, args, dryRun };
...
server.tool(
  ...
  "Run a project Python script through UnrealEditor-Cmd. Scripts must live under ASCENT_ROOT/scripts.",
  {
    script: z.string(),
...
server.tool(
  ...
  "Duplicate a /Game map asset. Defaults to dry-run and refuses overwrite unless requested.",
  {
    sourceMap: z.string(),
    targetMap: z.string(),
...
server.tool(
  ...
  "Load a map and assign a material to a named actor's StaticMeshComponent material slot.",
  {
    map: z.string(),
    actorLabel: z.string(),
    material: z.string(),
...
server.tool(
  "unreal_apply_lanin_gaea_lab_material",
  "Apply /Game/Environment/LaninTrueGaeaLab/M_LaninTrue_Play_GaeaLab to the LaninTrue_GaeaLab terrain.",
  {
    timeoutSeconds: z.number().int().positive().default(300),
    dryRun: z.boolean().default(true),
  },
...
server.tool(
  "uds_verify_mcp_weather_demo",
  "Verify /Game/Maps/mcp_playground points at the one-minute UDS weather choreography profile.",
  {
    timeoutSeconds: z.number().int().positive().default(300),
    dryRun: z.boolean().default(true),
  },
...
function udsPresetValues(name: z.infer<typeof udsPresetSchema>): Record<string, number | boolean> {
  switch (name) {
    case "dawn_alpenglow":
      return {
        "Time of Day": 6.1,
...
server.tool(
  "uds_inspect_level",
  "Load a map and inspect UDS-like sky actors and important time/weather properties.",
  {
    map: z.string(),
...
server.tool(
  "uds_apply_preset",
  "Apply a named UDS time/weather preset to a sky actor in one map.",
  {
    map: z.string(),
...
server.tool(
  ...
  "Report configured paths for this Ascent Unreal MCP server.",
  {},
  async () => toText({
    root,
    uproject,
