...
// ── apply-then-verify: discipline as infrastructure ──────────────
type Expectation = { prop: string; equals?: string | number | boolean; contains?: string; min?: number; max?: number };

export function evaluateExpectation(exp: Expectation, value: unknown): { pass: boolean; detail: string } {
  const num = typeof value === "number" ? value : Number.parseFloat(String(value));
  if (exp.equals !== undefined) {
    const pass =
      typeof exp.equals === "number" && Number.isFinite(num)
...
server.tool(
  ...
  "Run a repo script, then — in the SAME commandlet boot — probe an actor or asset and check " +
  ...
  async ({ script, args, verify, expect, allowWithEditorOpen, timeoutSeconds, dryRun }) => {
    ...
    try {
      scriptPath = resolveScript(script);
      if (verify.asset) assertGamePath(verify.asset, "verify.asset");
      if (verify.map) assertGamePath(verify.map, "verify.map");
...
`;
    ...
    if (dryRun) return raw;
    const parsed = JSON.parse(raw.content[0].text) as { ok: boolean; output?: string[] };
    const verifyLine = (parsed.output ?? []).find((l) => l.includes("MCP VERIFY_JSON "));
    let checks: Array<{ pass: boolean; detail: string }> = [];
    if (verifyLine) {
      try {
        const data = JSON.parse(verifyLine.slice(verifyLine.indexOf("MCP VERIFY_JSON ") + "MCP VERIFY_JSON ".length)) as {
...
server.tool(
  ...
  "One-call arc verification: set a TOD profile's driver to a fast FixedTimer, launch the map in " +
...
`;
    ...
    const flipParsed = JSON.parse(flip.content[0].text) as { ok: boolean; output?: string[] };
    if (!flipParsed.ok) return jsonResult(false, { tool: "weather_capture_phase_stills", stage: "flip-driver", inner: flipParsed });
    const phasesLine = (flipParsed.output ?? []).find((l) => l.includes("MCP PHASES_JSON "));
    const meta = JSON.parse(phasesLine!.slice(phasesLine!.indexOf("MCP PHASES_JSON ") + "MCP PHASES_JSON ".length)) as {
      orig_driver: string;
      orig_secs: number;
...
`;
    ...
    const result = await withLock(async () => {
      ...
      const killGame = () => gamePid && killTree(gamePid);
      const deadline = Date.now() + timeoutSeconds * 1000;
      const shots: Array<{ phase: string; file?: string; ok: boolean; detail?: string }> = [];
      try {
        // Anchor: the controller logs "PHASE TRANSITION: [-1] -> [0]" at run start.
        let t0: number | undefined;
        while (!t0 && Date.now() < deadline - 30_000) {
          await new Promise((r) => setTimeout(r, 1000));
          if (spawnError) return jsonResult(false, { tool: "weather_capture_phase_stills", error: `game launch failed: ${spawnError}` });
          try {
            const log = await fsp.readFile(logPath, "utf8");
            if (/PHASE TRANSITION: \[-1\]/.test(log)) t0 = Date.now();
          } catch {
            /* log not written yet */
          }
        }
        if (!t0) return jsonResult(false, { tool: "weather_capture_phase_stills", error: "run start never appeared in the log" });
        for (const ph of meta.phases) {
          ...
          const cap = await runProcess(
            "powershell",
            ["-NoProfile", "-ExecutionPolicy", "Bypass", "-File", ps1, "-OutFile", file, "-ConsoleCmds", "", "-GamePid", String(gamePid)],
            45_000,
          );
          shots.push({
            phase: ph.name,
    ...
    const restoreParsed = JSON.parse(restore.content[0].text) as { ok: boolean };
    const resultParsed = JSON.parse(result.content[0].text) as Record<string, unknown> & { ok: boolean };
    if (!restoreParsed.ok) {
      return jsonResult(false, {
        ...resultParsed,
        ok: undefined,
...
server.tool(
  "anim_audit_assets",
  "READ-ONLY animation audit for a skeleton: every AnimSequence (play length, rate scale, additive), " +
