...
if (!status.ok || typeof status.editorRunning !== "boolean") fail("status shape wrong");
...
if (!dry.ok || dry.dryRun !== true || !Array.isArray(dry.args)) fail("dry-run shape wrong");
if (!dry.args.some((a: string) => a.includes("-Unattended"))) fail("commandlet flags missing");

// The safety default: a mutating tool called WITHOUT dryRun must dry-run.
const mutDefault = JSON.parse(
  text(
    await client.callTool({
...
if (mutDefault.dryRun !== true) fail("mutating tool did not default to dryRun:true");
...
if (escape.ok !== false) fail("path escape was not rejected");

// v2.1 additions: screenshot dry-run shape + spawn source validation.
const shotDry = JSON.parse(
  text(
    await client.callTool({
...
if (!shotDry.ok || shotDry.dryRun !== true || !Array.isArray(shotDry.args)) fail("screenshot dry-run shape wrong");

const badSpawn = JSON.parse(
  text(
    await client.callTool({
...
if (badSpawn.ok !== false) fail("spawn source validation missing");
